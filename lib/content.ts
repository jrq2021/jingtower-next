...
export function createMockReading(
  ...
  return {
    ...
    freeResult: {
      ...
      currentState: scene.freeInsight,
      cardSynthesis: `牌面显示：${cardsWithOrientation[0]?.name ?? "—"}（${cardsWithOrientation[0]?.role ?? "现状"}）→ ${cardsWithOrientation[1]?.name ?? "—"}（${cardsWithOrientation[1]?.role ?? "阻力"}）→ ${cardsWithOrientation[2]?.name ?? "—"}（${cardsWithOrientation[2]?.role ?? "行动"}）。这三张牌提示你需要先看清现状中的模糊信号，再面对内心的阻力，最后迈出一步低风险行动。`,
      insight: scene.freeInsight,
      action: scene.freeAction,
      risk: "把直觉猜测当成事实，会陷入自我实现的预言。先收集信息，再下判断。",
